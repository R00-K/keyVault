...
class ReceiveTrustNfcScreen extends StatelessWidget {
  const ReceiveTrustNfcScreen({super.key});
          ...
          KvSectionCard(
            child: Column(
              ...
              children: [
                ...
                const Text(
                  'This screen will wait for trust material from a nearby tap.',
                  style: AppTextTheme.bodyMuted,
                ),
                const SizedBox(height: 12),
                const Text(
                  'Future steps: read NFC trust material, verify identity, derive trust state, then create a trusted contact.',
                  style: AppTextTheme.caption,
                ),
                const SizedBox(height: 18),
                KvButton(
                  label: 'Receive NFC Trust',
                  ...
                  onPressed: () async {
                    ...
                    if (context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text(
                            'NFC trust receiving is a placeholder.',
                          ),
                        ),
                      );
                    }
